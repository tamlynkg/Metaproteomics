...
outdir=paste('LimmaKEGG',sep='')

dir.create(outdir, showWarnings = TRUE, recursive = FALSE, mode = "0777")

counts <- read.delim("BV_Limma_Kegg.txt", header = TRUE, sep='\t', row.names = 1)
...
pval_lists <- vector("list", length(colnames(contrasts)))
qval_lists <- vector("list", length(colnames(contrasts)))

pval_lists_0_005 <- vector("list", length(colnames(contrasts)))

for ( i in seq_along(colnames(contrasts))) {
  cntrst = colnames(contrasts)[i]
  cntrst_ <- strsplit(cntrst, '-')
  Positive <- cntrst_[[1]][1]
  Negative <- cntrst_[[1]][2]
...
for(i in seq_along(hclust_methods)) {
  ...
  labels_colors(tmp_dend) <- rainbow_hcl(6)[sort_levels_values(as.numeric(i)[order.dendrogram(tmp_dend)])]
  par(mar = c(15,3,3,3))
  plot(tmp_dend)
  dev.off()
...
#counts <- read.delim("BV_Genera_Heatmap.txt", header = TRUE, sep = "\t", row.names = 1)
#input <- as.matrix(t(counts))
library('dendextend')
#dend_r <- df %>% dist(method="man") %>% hclust(method="ward.D") %>% as.dendrogram %>% ladderize %>% color_branches(k=10)
dend_c <- t(input) %>% dist(method="euclidean") %>% hclust(method="ward.D") %>% as.dendrogram %>% ladderize%>% color_branches(k=3)
...
legend(0.9, 0.9, legend = c("High", "Low", "Medium"), fill=c('#FF0000', '#228B22', '#0000FF'), cex = 0.5)  
  dev.off() }
...
hm(input, heatmap_path ,"heatmap_iBAQ.jpeg", "Fungal Proteins Between Inflammation States", "log2(iBAQ intensity)", dend_c, dend_r)
# Strain comparisons
...
pc <- function( df, path, file, var.axes ) {
  ...
  pca <- prcomp(counts, center=TRUE, scale=TRUE)
  g <- ggplot(pca, obs.scale = 1, var.scale = 1,, ellipse=TRUE, circle=TRUE, var.axes=var.axes)
...
str(counts)
pc(counts, pca_path, "all_identified_pca.png", FALSE)
fortify(counts)
